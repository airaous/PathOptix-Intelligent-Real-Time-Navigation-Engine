...
const AdvancedFeaturesPanel = ({ 
  ...
  // Update eco metrics when eco mode is enabled
  useEffect(() => {
    if (features.ecoMode && route) {
      const metrics = {
        fuelSavings: Math.random() * 20 + 5, // 5-25%
        co2Reduction: Math.random() * 30 + 10, // 10-40%
        costSavings: Math.random() * 15 + 3, // $3-18
  ...
  useEffect(() => {
    if (features.voiceNavigation && 'speechSynthesis' in window) {
  ...
  return (
    <div className={`bg-white rounded-xl shadow-lg border border-gray-200 max-h-[calc(100vh-6rem)] overflow-hidden flex flex-col ${className}`}>
      {/* Header */}
